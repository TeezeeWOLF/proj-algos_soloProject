...
from flask_app.models import user, journal # import entire file, rather than class, to avoid circular imports
...
def create_user():
    if not user.User.validate_user_data(request.form): return redirect('/')
    user.User.create_users(request.form)
    return redirect('/home')


@app.post('/users/login')
def users_login():
    if user.User.login(request.form): return redirect('/home')
    return redirect('/')
...
def affirmation():
    if 'user_id' not in session: return redirect('/')
    affirmation = journal.Journal.get_all_affirmations()
    return render_template('affirmation.html', affirmation = affirmation)
